...
def render_motion_in_scene(
    ...
    for i in range(len(vertices)):
        ...
        if args.output_obj == 'True':
            obj_path = save_folder.replace('imgs/','')
            if not os.path.exists(obj_path):
                os.makedirs(obj_path)
            obj_path = os.path.join(obj_path,'frame_obj_{}.obj'.format(i))
            with open(obj_path, 'w') as fw:
                body.export(fw, 'obj')
