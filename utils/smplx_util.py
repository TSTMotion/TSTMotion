...
class SMPLX_Util():
    ...
    def convert_smplx_verts_transfomation_matrix_to_body(T, trans, orient, pelvis):
        ...
        trans = np.matmul(R, pelvis) + trans - pelvis
    ...
    @staticmethod
    def get_body_vertices_sequence(smplx_folder: str, pkl: Tuple, num_betas=16, model_type='smpl'):
        ...
        """
        if len(pkl) == 4:
            trans, orient, betas, body_pose = pkl

            seq_len = len(body_pose)
            body_model = smplx.create(smplx_folder, model_type=model_type,
