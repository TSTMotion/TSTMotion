...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, mode, control_joint=0, density=100):
        ...
        if 'HumanML3D' in opt.data_root:
            spatial_norm_path = './OmniControl/dataset/humanml_spatial_norm'
        elif 'KIT' in opt.data_root:
            spatial_norm_path = './OmniControl/dataset/kit_spatial_norm'
        else:
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", control_joint=0, density=100, **kwargs):
        ...
        abs_base_path = f'./OmniControl'
        # dataset_opt_path = pjoin(abs_base_path, datapath)
        dataset_opt_path = './OmniControl/dataset/humanml_opt.txt'
        device = None  # torch.device('cuda:4') # This param is not in use in this context
        opt = get_opt(dataset_opt_path, device)
        opt.meta_dir = pjoin(abs_base_path, opt.meta_dir[2:])
        # opt.motion_dir = pjoin(abs_base_path, opt.motion_dir)
        # opt.text_dir = pjoin(abs_base_path, opt.text_dir)
        opt.model_dir = pjoin(abs_base_path, opt.model_dir) # ./motion_diffusion_model/./checkpoints/t2m/Comp_v6_KLD01/model
        opt.checkpoints_dir = pjoin(abs_base_path, opt.checkpoints_dir) # ./motion_diffusion_model/./checkpoints
        opt.save_root = pjoin(abs_base_path, opt.save_root)
        # print(opt.save_root) # ./motion_diffusion_model/./checkpoints/t2m/Comp_v6_KLD01
        opt.meta_dir = './OmniControl/dataset'
        self.opt = opt
