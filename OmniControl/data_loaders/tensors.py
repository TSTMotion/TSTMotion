...
def collate(batch):
    ...
    lenbatchTensor = torch.as_tensor(np.array(lenbatch))
    ...
    if 'action' in notnone_batches[0]:
        ...
        cond['y'].update({'action': torch.as_tensor(np.array(actionbatch)).unsqueeze(1)})
    ...
    if 'obj_points' in notnone_batches[0]:
        ...
        cond['y'].update({'obj_points': torch.as_tensor(np.array(obj_points))})
